<template>
  <menu
    class="absolute left-0 bg-menu transform transition-all duration-750
    lg:px-8 sm:px-6 px-4 md:pt-10 ot-5 md:pb-40 pb-20 xl:w-1/3 lg:w-2/5 sm:w-1/2 w-full m-0"
    :class="{
  ...
  >
    <ul class="text-menu uppercase md:text-xl sm:text-base text-sm">
      ...
      >
        ...
        >
          <li class=" mt-6">
            <nuxt-link :to="localePath('/main-menu')" @click.native="$emit('closeMenu')">
              {{ $t('navigation.item3') }}
            </nuxt-link>
          </li>

          <li class="mt-4">
            <nuxt-link :to="localePath('/lunch-menu')" @click.native="$emit('closeMenu')">
              {{ $t('navigation.item4') }}
            </nuxt-link>
          </li>
        </ul>
      ...
      <li class="my-6">
        <nuxt-link :to="localePath('/poster')" @click.native="$emit('closeMenu')">
          {{ $t('navigation.item5') }}
        </nuxt-link>
      </li>
      <li class="my-6">
        <nuxt-link :to="localePath('/reports')" @click.native="$emit('closeMenu')">
          {{ $t('navigation.item6') }}
        </nuxt-link>
      </li>
      <li class="my-6">
        <nuxt-link :to="localePath('/contacts')" @click.native="$emit('closeMenu')">
          {{ $t('navigation.item7') }}
        </nuxt-link>
      </li>
    </ul>
